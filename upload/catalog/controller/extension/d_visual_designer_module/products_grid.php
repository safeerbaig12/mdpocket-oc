...
class ControllerExtensionDVisualDesignerModuleProductsGrid extends Controller
{
    private $codename = 'products_grid';
    private $route = 'extension/d_visual_designer_module/products_grid';
    ...
    public function index($setting)
    {
        switch ($setting['mode']) {
            case 'latest':
                $products = $this->model_extension_d_visual_designer_module_products_grid->getLatestProducts($setting['count_product']);
                break;

            case 'special':
                $products = $this->model_extension_d_visual_designer_module_products_grid->getProductSpecials($setting['count_product']);
                break;

            case 'popular':
                $products = $this->model_extension_d_visual_designer_module_products_grid->getPopularProducts($setting['count_product']);
                break;
            case 'bestseller':
                $products = $this->model_extension_d_visual_designer_module_products_grid->getBestSellerProducts($setting['count_product']);
                break;
            case 'custom_categories':
                $products = array();
                
                if(!empty($setting['categories'])) {
                    $products = $this->model_extension_d_visual_designer_module_products_grid->getCategoryProducts($setting['count_product'], $setting['categories']);
                }
                break;
        ...
        if ($products) {
            foreach ($products as $result) {
                ...
                $data['products'][] = array(
                    ...
                    'tax' => $tax,
                    'rating' => $rating,
                    'wishlist' => $this->model_extension_d_visual_designer_module_products_grid->hasWishList($result['product_id']),
                    'attribute_groups' => $this->model_extension_d_visual_designer_module_products_grid->getProductAttributes($result['product_id']),
                    'href' => $this->url->link('product/product', 'product_id=' . $result['product_id'])
                );
            }
        }
